...
ItemSection
            {
                ...
                title:  qsTr("What Email do you want to use to create a wallet?")
                subtitle: qsTr("If more than one person will use this computer, you can create multiple wallet after installation. But, remember you can only assign limited amount of root token to your arbitrary users.")

                TextField
                {
                    id: _userEmailField
                    enabled: config.isEditable("Email")
                    width: parent.width
                    placeholderText: qsTr("Your Email")
                    text: config.Email
                    onTextChanged: config.setEmail(text)
                    ...
                    background: Rectangle
                    {
                        ...
                        radius: 5
                        opacity: 0.5
                        border.color: _userLoginField.text.length ? Kirigami.Theme.backgroundColor : (config.EmailStatusChanged ? Kirigami.Theme.backgroundColor : Kirigami.Theme.negativeTextColor)
...
ItemSection
            {
                ...
                title:  qsTr("Do you want to use Biconomy?")
                subtitle: qsTr("Feel free to click on the button below if you need to use Biconomy in order to create a wallet!")
